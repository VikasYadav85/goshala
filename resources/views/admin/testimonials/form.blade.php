...
<form method="POST" action="{{ $testimonial->exists ? route('admin.testimonials.update', $testimonial) : route('admin.testimonials.store') }}" enctype="multipart/form-data" class="admin-card p-6 max-w-2xl">
    ...
    <div class="grid sm:grid-cols-2 gap-4">
        <div><label class="form-label">Name *</label><input name="name" required value="{{ old('name', $testimonial->name) }}" class="form-input"></div>
        <div><label class="form-label">Role</label><input name="role" value="{{ old('role', $testimonial->role) }}" placeholder="Donor / Volunteer / Devotee" class="form-input"></div>
        <div><label class="form-label">Location</label><input name="location" value="{{ old('location', $testimonial->location) }}" class="form-input"></div>
        <div><label class="form-label">Rating *</label><select name="rating" class="form-select">@for ($i=1;$i<=5;$i++)<option value="{{ $i }}" @selected(old('rating', $testimonial->rating) == $i)>{{ $i }} star{{ $i > 1 ? 's' : '' }}</option>@endfor</select></div>
