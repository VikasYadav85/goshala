...
<div class="grid lg:grid-cols-3 gap-6 mt-4">
    <div class="lg:col-span-2 admin-card p-6">
        ...
        <dl class="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
            <dt class="text-gray-500">Email</dt><dd>{{ $volunteer->email }}</dd>
    ...
    <form method="POST" action="{{ route('admin.volunteers.update', $volunteer) }}" class="admin-card p-6 h-fit">
        @csrf @method('PATCH')
        ...
        <textarea name="admin_notes" rows="4" class="form-textarea mb-4">{{ $volunteer->admin_notes }}</textarea>
