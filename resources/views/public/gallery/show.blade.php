    ...
    <div class="container mx-auto px-4">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse ($album->items as $item)
                @php
                    $isImage = $item->type === 'image' && $item->file_path;
                    // Anything with an external URL but no image renders as a video,
                    // even if its type was saved incorrectly.
                    $isVideo = ! $isImage && $item->external_url;
                @endphp
                @if ($isImage || $isVideo)
                    <figure class="card-soft overflow-hidden">
                        ...
                        @else
                            <div class="aspect-video">
                                <iframe class="w-full h-full" src="{{ $item->embed_url }}" frameborder="0" allowfullscreen loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe>
                            </div>
                        @endif
                        @if ($item->caption)
